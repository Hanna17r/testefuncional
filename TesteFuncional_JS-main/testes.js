...
const function5 = (input) => {
  if (!/^[a-zA-Z]+$/.test(input)) {
    throw new Error('funcaoCaixaBaixa: Erro! Entrada inválida. Deve ser composta apenas por letras.');
  } else {
    const result = funcaoCaixaBaixa(input);
    return `funcaoCaixaBaixa: Recebida a palavra "${input}". A palavra em minúscula é "${result}".`;
  }
};
...
const testFunction4 = () => {
  const userInput = prompt('Digite um número inteiro para a Função 4:');
  try {
